...
render = (array) => {
  ...
  array.forEach((sv) => {
    ...
    stringHTML += `
        ...
        <button class="btn btn-success rounded" onclick="chinhSua(${
          newSinhVien.maSinhVien
        })">Chỉnh sửa</button>
        </td>
        </tr>
...
chinhSua = (maSV) => {
  ...
  sinhVien.forEach((key, index) => {
    if (key.maSV === maSV) indexSua = index;
  });
  if (indexEdit !== -1) {
