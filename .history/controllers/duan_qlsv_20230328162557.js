...
xoaSinhVien = (maSV) => {
  ...
  sinhVien.forEach((key, index) => {
    if (+key.maSinhVien === maSV) indexXoa = index;
  });
  if (indexXoa !== -1) sinhVien.splice(indexXoa, 1);
  saveStorage();
  render(sinhVien);
};
// Chỉnh sửa:
...
// Search tên sinh viên
document.querySelector("#txtTuKhoa").oninput = function () {
  let tuKhoa = document.querySelector("#txtTuKhoa").value.trim();
  let convertTuKhoa = stringToSlug(tuKhoa);
  let searchSinhVien = [];
  sinhVien.forEach((key) => {
    if (stringToSlug(key.tenSinhVien.trim()).search(convertTuKhoa) !== -1)
      searchSinhVien.push(key);
  });
  render(searchSinhVien);
};
// Set local storage
saveStorage = () => {
