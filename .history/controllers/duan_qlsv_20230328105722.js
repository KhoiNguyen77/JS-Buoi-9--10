...
render = (array) => {
  ...
  array.forEach((sv) => {
    stringHTML += `
        ...
        <th>${sv.diemTrungBinh}</th>
        <th>
        <button class="btn btn-danger rounded" onclick="xoaSinhVien(${array}, ${sv.maSinhVien})">Xóa</button>
...
getStorage = () => {
  if (localStorage.getItem("Mảng Sinh Viên")) {
    var arraySinhvien = JSON.parse(localStorage.getItem("Mảng Sinh Viên"));
...
// Xóa SV:
xoaSinhVien = (array, maSV) => {
  array.forEach((key) => {
    if (key.maSinhVien === maSV) {
      sinhVien.splice(key.maSinhVien, 1);
    }
    render(sinhVien);
  });
};
